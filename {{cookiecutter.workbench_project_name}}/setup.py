...
import os
# from freecad.{{cookiecutter.workbench_dir_name}}.version import __version__
# name: this is the name of the distribution.
# Packages using the same name here cannot be installed together

version_path = os.path.join(os.path.abspath(os.path.dirname(__file__)), 
                            "freecad", "{{cookiecutter.workbench_dir_name}}", "version.py")
with open(version_path) as fp:
    exec(fp.read())

setup(name='freecad.{{cookiecutter.workbench_dir_name}}',
      version=str(__version__),
      packages=['freecad',
                'freecad.{{cookiecutter.workbench_dir_name}}'],
